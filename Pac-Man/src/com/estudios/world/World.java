...
import com.estudios.entities.Fruit;
			...
			for(int xx = 0; xx < map.getWidth(); xx++) {
				for(int yy = 0; yy < map.getHeight(); yy++) {
					...
					if(pixelAtual == 0xFF000000) {
						...
						//Maçã
						Fruit fruit = new Fruit(xx*16, yy*16, 16, 16, 0, Entity.MACA);
						Game.entities.add(fruit);
						Game.contFruit++;
					} else if(pixelAtual == 0xFFFF6A00) {
						//Inimigo 1
						Enemy en = new Enemy(xx*16, yy*16, 16, 16, 1, Entity.ENEMY1);
						Game.entities.add(en);
					} else if(pixelAtual == 0xFF00FFFF) {
						Enemy en = new Enemy(xx*16, yy*16, 16, 16, 1, Entity.ENEMY2);
						Game.entities.add(en);
					}
